...
EPROC_TRIBUNAIS = [
    {
        "nome": "TJSP",
        "usuario_env": "EPROC_SP_USUARIO",
        "senha_env": "EPROC_SP_SENHA",
        "totp_secret_env": "EPROC_SP_TOTP_SECRET",
        "url_portal": "https://www.tjsp.jus.br/eproc",
        "modo_link": "tjsp",
        # 2º clique (depois de "1º Grau") vai para essa URL — usada
        # tanto para achar o link quanto como fallback direto.
        "url_direto_click": "https://eproc1g.tjsp.jus.br/eproc/",
        "url_direto": (
            "https://sso.tjsp.jus.br/realms/eproc/protocol/openid-connect/auth"
            "?kc_idp_hint=tjsp"
            "&eproc_client_id=eproc1g.tjsp.jus.br"
            "&response_type=code"
            "&redirect_uri=https%3A%2F%2Feproc1g.tjsp.jus.br%2Feproc%2Fexterno_controlador.php%3Facao%3DSSO%2Fcallback"
            "&client_id=eproc1g.tjsp.jus.br"
            "&nonce=9335eb579ce05e535d45af3db36f2623"
            "&state=814dbda36637e839b5beee7b286a837a"
            "&scope=profile+openid"
        ),
        "texto_link_regex": re.compile(
            r"C[íi]vel\s*[–-]\s*Jec\s*[–-]\s*Cejuscs\s*[–-]\s*Registros\s*P[úu]blicos"
            r"\s*[–-]\s*Fal[êe]ncia\s*[–-]\s*Empresarial",
            re.IGNORECASE,
        ),
        "usa_certificado": False,  # só login e senha, sem certificado
        "tipo_pesquisa": "OAB",
        "valores_busca": [
            "SP114904",
            "SP295139",
...
def diagnosticar_tela(pagina, rotulo):
    ...
    try:
        os.makedirs("debug_eproc_sp", exist_ok=True)
        agora = datetime.now().strftime("%Y%m%d_%H%M%S")
        nome_arquivo = f"debug_eproc_sp/{rotulo}_{agora}.png".replace(" ", "_")
...
def _clicar_acesso_tjsp(sessao, tribunal):
    """
    TJSP tem fluxo de DOIS cliques:
    1º: <a class="tjsp-Eproc-bg-PrimaryColor">1º Grau</a>
    2º: <a class="c1" href="https://eproc1g.tjsp.jus.br/eproc/">
          Cível – Jec – Cejuscs – Registros Públicos – Falência –
          Empresarial</a>
    """
    ...
    # ---------- Passo 1: "1º Grau" ----------
    try:
        pagina.wait_for_selector("a.tjsp-Eproc-bg-PrimaryColor", state="visible", timeout=15000)
    except Exception:
        pass

    alvo_1_grau = pagina.locator("a.tjsp-Eproc-bg-PrimaryColor")
    if alvo_1_grau.count() == 0:
        alvo_1_grau = pagina.get_by_text(re.compile(r"^\s*1[ºo]?\s*Grau\s*$", re.IGNORECASE))

    if alvo_1_grau.count() == 0:
        return None, False

    try:
        alvo_1_grau.first.scroll_into_view_if_needed(timeout=5000)
        alvo_1_grau.first.click()
    except Exception:
        return None, False
    ...
    # ---------- Passo 2: opção de competência ----------
    seletor_href = f"a[href='{tribunal['url_direto_click']}']"
    try:
        pagina.wait_for_selector(f"{seletor_href}, a.c1", state="visible", timeout=15000)
    except Exception:
        pass

    alvo_clique = pagina.locator(seletor_href)
    if alvo_clique.count() == 0:
        alvo_clique = pagina.locator("a.c1").filter(has_text=texto_link)
    if alvo_clique.count() == 0:
...
def localizar_e_clicar_acesso(sessao, tribunal):
    return _clicar_acesso_tjsp(sessao, tribunal)
